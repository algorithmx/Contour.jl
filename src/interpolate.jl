...
@inline function _construct_vertex(xv, yv, ::Type{VT}) where {VT}
    if VT <: NTuple{2,T} where {T<:Integer}
        T = VT.parameters[1]
        return (round(T, xv), round(T, yv))
    elseif VT <: Tuple
        return (xv, yv)
    else
        return VT(xv, yv)
...
function interpolate(x, y, z::AbstractMatrix, h::Number, ind, edge::UInt8, ::Type{VT}) where {VT}
    ...
    return _construct_vertex(x_interp, y_interp, VT)
end
